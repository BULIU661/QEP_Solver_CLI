...
├── bin/                          预编译可执行文件及运行时 DLL
├── Problems/                     测试问题数据（.bin / .txt / problem.json）
...
输出：`bin/qep.exe`（Windows）/ `bin/qep`（Linux）
...
字符串数组，列出用例名称。程序会自动在 `problem_base_path` 下扫描匹配的目录并加载 `problem.json`。留空则仅使用 `active_cases`。

| 字段 | 说明 |
|---|---|
| `test_cases` | 字符串数组，每项为例名称（对应 `Problems/` 下的子目录名） |
| `active_cases` | 当前激活的用例列表 |
...
| `QEP_BUILD_GUI` | OFF | 构建 Qt GUI（GUI 代码不在本仓库中） |
...
Copyright (c) 2025 QEP Solver Contributors
